# **IBM watsonx Code Assistant**</br>*Modernizing Applications*
...
The core curriculum for **watsonx Code Assistant: Modernizing Applications (*W7S220G*)** is segmented across two chapters:

1. **Modernizing WebSphere to Liberty**: the details of which are outlined and hypedlinked in the table above. It is recommended that participants start with the *[1. Objectives and Requirements](https://ibm.github.io/wca-modernize/appmod/1/)* module first and work their way sequentially through the material from there.

2. **IBM Cloud (SaaS) Configuration**: an ```optional``` chapter prepared for those who would like to replicate the configuration of a WCA environment on their local machines. These configuration steps will enable participants to authorize VS Code or Eclipse IDEs for use with *IBM watsonx Code Assistant* via extensions (or plug-ins), bypassing the need of working through a virtual machine. Instructions are available for doing so on both macOS and Windows operating system-based machines.
...
!!! tip ""
    **Record** information to a notepad for future reference when prompted to do so by "green" call-out boxes such as these. The on-screen details will be valuable for the final evaluation quiz.