...
1. Ensure that the **Explorer** tab is open to the `wca-assets-2025` directory. If it is not, you can locate those assets within the `Documents` directory of the VM.

    Participants will now need to build the application *Project*.

---

2. Open a **Terminal** within VS Code (if you haven't already) by clicking **Terminal** > **New Terminal** from the top taskbar.

    - The console will automatically open to the root of the `wca-assets-2025` directory
    - Drill down into the `/1-java-modernization/modresorts-twas-j8/was_dependency` subdirectory
    - **Attention:** Substitute the `<REPLACE>` placeholder with the path to *your* directory
    ``` shell
    cd 1-java-modernization/modresorts-twas-j8/was_dependency
    ```
    - Source the `~/.zshrc` *PATH* and *JAVA_HOME* variables by executing the following:
    ``` shell
    source ~/.zshrc
    ```
...
3. With the `was_dependency` folder now prepared with the necessary drivers, you are ready to *build* and *deploy* the application project into a live web application
...
4. Within the **LIBERTY DASHBOARD** , a `modresorts` app will show up under the *Liberty Dashboard* panel.
