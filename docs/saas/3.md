...
1. **Download** and **install** the `v21` of the *Java Developer Kit* (JDK) using the vendor link below.
    ...
    - It is strongly recommended that you **install JDK 21** (or as early as JDK 11)
    - Newer-generation MacOS users will probably want to select the *ARM64 DMG Installer*
...
3. Set the `JAVA_HOME` environment variable.
    ...
    - **macOS:** Edit the shell configuration file
        - Default shell (`zsh`) with Terminal:
        ``` shell
        nano ~/.zshrc
        ```
